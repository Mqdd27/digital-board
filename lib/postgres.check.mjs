...
try {
  ...
  await run("INSERT INTO tasks (id,column_id,title,label,priority,assignee_id,due_date,position,created_at) VALUES (?,?,?,?,?,?,?,0,?)",
    t1, c1, "Task one", "QA", "high", ada, today, now);
  await run("INSERT INTO tasks (id,column_id,title,label,priority,assignee_id,due_date,position,created_at) VALUES (?,?,?,?,?,?,?,1,?)",
    t2, c1, "Task two", null, "low", null, null, now);
  ...
  await run("INSERT INTO attachments (id,message_id,name,mime,size,created_at) VALUES (?,?,?,?,?,?)",
    att, m2.id, "f.png", "image/png", 10, now);
  const files = await all(
    `SELECT id, message_id, name, mime, size FROM attachments WHERE message_id IN (?,?) ORDER BY created_at, id`,
    m1.id, m2.id);
